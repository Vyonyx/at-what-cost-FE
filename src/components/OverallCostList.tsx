...
function OverallCostList() {
  ...
  const expenseTransactions = transactions.filter(item => Number(item['Amount']) < 0)
  const incomeTransactions = transactions.filter(item => Number(item['Amount']) > 0)
  
  let expensesTotal = expenseTransactions
    .reduce((prev, item) => prev + Number(item['Amount']), 0)
  expensesTotal = Math.round((expensesTotal * 100) / 100)

  let incomeTotal = incomeTransactions
    .reduce((prev, item) => prev + Number(item['Amount']), 0)
